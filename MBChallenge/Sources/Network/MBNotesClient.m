...
@interface MBNotesClient ()

@property (nonatomic, strong, readwrite) NSArray *jsonNotes;

@end

@implementation MBNotesClient
{
}
...
- (id)init
{
    ...
    if ( self )
    {
        self.jsonNotes = @[@{ @"id":@1, @"text": @"First note" },
...
{
    NSLog(@"fetching notes");
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        [NSThread sleepForTimeInterval:2.0f];
        NSLog(@"fetching notes done.");

        if (completion)
        {
            completion([NSArray arrayWithArray:self.jsonNotes], nil);
        }
